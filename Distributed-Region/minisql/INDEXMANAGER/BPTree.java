...
public class BPTree<K extends Comparable<? super K>, V> { // K:key type; V:value type
    ...
    public void check_structure() throws RuntimeException { // used for debug: to check the legality of the structure of
                                                            // the B+ tree
    ...
    static abstract class Node<K extends Comparable<? super K>, V> {
        ...
        protected int find_index(K key) { // binary-search
            ...
            if (key.compareTo(this.keys[index]) > 0) {
                index++; // if key not found, return the index of the first key whose value is no less
                         // than the key to find
            }
            return index;
        }
    ...
    static class InternalNode<K extends Comparable<? super K>, V> extends Node<K, V> {
        ...
        private InternalNode<K, V> insert_and_split(K newKey, Node<K, V> newChild, int index) { // insert the new child
                                                                                                // with the new key into
                                                                                                // the node at the
                                                                                                // specific index and
        ...
        private Node<K, V> delete_leaf_node(LeafNode<K, V> node, K key) throws IllegalArgumentException { // delete a
                                                                                                          // leaf node
                                                                                                          // which is
                                                                                                          // this's
                                                                                                          // child
            ...
            if (left != null && left.cnt > this.order / 2) { // move a children from left sibling to this
                ...
                this.children[0] = left.children[left.cnt]; // change the last children of left sibling to the first
                                                            // children of this
            ...
            if (right != null && right.cnt > this.order / 2) { // move a children from right sibling to this
                ...
                this.children[this.cnt + 1] = right.children[0]; // change the first children of left sibling to the
                                                                 // last children of this
                ...
                right.cnt--;
                right.update_deleted_keys(this.keys[this.cnt - 1], right.children[0].keys[0]); // update the keys of
                                                                                               // right's ancestors
                return null;
            }
            ...
            left.merge(right);
            return this.parent.delete_internal_node(right); // after merging right, right should be deleted from its
                                                            // parent
        }
        ...
        private Node<K, V> delete_internal_node(InternalNode<K, V> node) throws IllegalArgumentException { // delete an
                                                                                                           // internal
                                                                                                           // node which
                                                                                                           // is this's
                                                                                                           // child
            ...
            if (left != null && left.cnt > this.order / 2) { // move a children from left sibling to this
                ...
                this.parent.keys[this.parent.find_node_index(this) - 1] = left.keys[left.cnt - 1]; // update the keys of
                                                                                                   // this's parent
                this.children[1] = this.children[0];
                this.children[0] = left.children[left.cnt]; // change the last children of left sibling to the first
                                                            // children of this
            ...
            if (right != null && right.cnt > this.order / 2) { // move a children from right sibling to this
                this.keys[this.cnt] = right.parent.keys[right.parent.find_node_index(right) - 1];
                right.parent.keys[right.parent.find_node_index(right) - 1] = right.keys[0]; // update the keys of
                                                                                            // right's parent
                this.children[this.cnt + 1] = right.children[0]; // change the first children of left sibling to the
                                                                 // last children of this
                this.children[this.cnt + 1].parent = this;
    ...
    static class LeafNode<K extends Comparable<? super K>, V> extends Node<K, V> {
        ...
        private LeafNode<K, V> insert_and_split(K newKey, V newValue, int index) { // insert the new value with the new
                                                                                   // key into the node at the specific
                                                                                   // index and split the node into 2
        ...
        public Node<K, V> delete(K key) throws IllegalArgumentException {
            ...
            left.merge(right);
            return this.parent.delete_leaf_node(right, oldKey); // after merging right, right should be deleted from its
                                                                // parent
        }
