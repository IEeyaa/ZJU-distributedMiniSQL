...
public class BufferManager {
    ...
    public static void destruct_buffer_manager() {
        for (int i = 0; i < MAXBLOCKNUM; i++)
            if (buffer[i].valid())
                write_block_to_disk(i); // write back to disk if it's valid
    }
    ...
    // if the block exist and it's valid, return this block else return a empty
    // block
    public static int read_block_from_disk(String filename, int ofs) {
        ...
        for (i = 0; i < MAXBLOCKNUM; i++) // find the target block
            if (buffer[i].valid() && buffer[i].get_filename().equals(filename)
                    && buffer[i].get_block_offset() == ofs)
                return i;
        File file = new File(filename); // block does not found
        int bid = get_free_block_id();
        try {
            if (bid == EOF)
                return EOF; // there are no free blocks
            if (!file.exists())
                file.createNewFile(); // if not exists such file
            if (!read_block_from_disk(filename, ofs, bid))
                return EOF;
        } catch (Exception e) {
            return EOF;
        }
    ...
    public static Block read_block_from_disk_quote(String filename, int ofs) {
        ...
        if (i < MAXBLOCKNUM) { // there exist a block
            ...
            int bid = get_free_block_id();
            if (bid == EOF || !file.exists())
                return null; // there are no free blocks
            if (!read_block_from_disk(filename, ofs, bid))
                return null;
            return buffer[bid];
        }
    ...
    private static boolean write_block_to_disk(int bid) {
        ...
        try {
            ...
            if (!out.exists())
                out.createNewFile(); // if file does not exist
